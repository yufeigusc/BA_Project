...
```{r}
fitControl = trainControl(method = "cv", number = 3, classProbs = TRUE, summaryFunction = twoClassSummary, verboseIter = TRUE, allowParallel = TRUE)
```

```{r}
ntree_list = c(400)
cur_nodesize_list = c(5)
...
for (cur_ntree in ntree_list) {
  for (cur_nodesize in cur_nodesize_list) {
    mtryGrid = expand.grid(.mtry = seq(6, 8, by = 1))
    set.seed(123)
