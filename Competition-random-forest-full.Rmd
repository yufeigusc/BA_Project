...
ntree = 900, nodesize = 5) 10    0.9458938
```{r}
fitControl = trainControl(method = "cv", number = 5, classProbs = TRUE, summaryFunction = twoClassSummary, allowParallel = TRUE, verboseIter = TRUE)
#mtryGrid = expand.grid(.mtry = seq(5, sqrt(ncol(data)) + 4, by = 1))
mtryGrid = expand.grid(.mtry = seq(8, 11, by = 1))
set.seed(123)
rfModel = train(formula1, data = train, method = "rf", metric = "ROC", trControl = fitControl, tuneGrid = mtryGrid, ntree = 900, nodesize = 5)
print(rfModel)
