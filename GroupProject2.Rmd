...
- multiplication on local_bo_qty and pieces_past_due help a little
  overdue_factor = local_bo_qty * pieces_past_due 0.8823446
```{r}
...
crossv(formula, temp_data6)
```
- can remove forecast_9_month and forecast_3_month 0.8824342
...
- add inventory_compliance
0.8898747

- remove overdue factor
0.8899593
...
- add Safety Stock Ratio: min_bank/(sales_3_month/30): 0.8889989 
- not helping
```{r}
...
crossv(
  formula, temp_data9)
```
- remove risk_flag?? 0.8949872


- Inventory aging 
- no

- variation of sales - too bad
```{r}
# Load the required packages
library(matrixStats)

# Assuming your dataframe is named 'df'
sales_3 = train$sales_3_month
sales_6 = train$sales_6_month - train$sales_3_month
sales_9 = train$sales_9_month - train$sales_6_month



# Select the columns for which you want to calculate mean and standard deviation
columns_to_calculate <- c("sales_3_month", "sales_6_month", "sales_9_month")

# Calculate mean and standard deviation for each row
mean_values <- rowMeans(cbind(sales_3, sales_6, sales_9), na.rm = TRUE)
sd_values <- rowSds(as.matrix(cbind(sales_3, sales_6, sales_9)), na.rm = TRUE)

temp_data10 = temp_data9

# Create a new column in the dataframe with mean and standard deviation values
temp_data10$mean_sd_sales <- sd_values/(mean_values+1)
missing_values <- any(is.na(temp_data10$mean_sd_sales))
...
- scale lead time and in transit qty 
- doesnt help
